...
# logging.basicConfig(level=logging.WARNING, format='%(asctime)s UTC %(levelname)s %(module)s(%(funcName)s) [%(process)d-%(thread)d-%(threadName)s]: %(message)s')
logging.basicConfig(level=logging.WARNING, format='%(asctime)s UTC : %(message)s')
...
logging.warning("Starting server on port %s " % args.port)
